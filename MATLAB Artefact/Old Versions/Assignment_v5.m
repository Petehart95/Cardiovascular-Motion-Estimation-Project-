...
mv_x = zeros([blockCount 1]);
mv_y = zeros([blockCount 1]);
...
for fr=1:2%200
    ...
    for i=110:C:size(Fr1,1)-40
        for j=110:R:size(Fr1,2)-20
            ...
            mv_x(row_mid) = row_mid;
            % Vector for storing new y values
            mv_y(col_mid) = col_mid;
            ...
            if (SAD < best_SAD)
                best_SAD = SAD;
                mv_x(row_mid) = row_mid-R;
                mv_y(col_mid) = col_mid-C;
            end
            %B2 Check
